...
String text;
String[] pieces = {
  "", "Your brain can predict \nthe future", "In fact, it's making \nlots of little predictions \nright now", "By making guesses about what comes next, the brain knows", "where to look", "when to walk", "and how to talk.", "mumble", 
...
void setup() {
  ...
  smooth();
   fontMid = loadFont("NimbusSanNov-Med-30.vlw");
  ...
  backImgPos.x = 100; //THEORETICALLY
...
//  vid=true;
  if (vid) {
    ...
    for (int i = 0; i< screenCount; i++) {
      ...
      image(myMovie, width/2, height/2-20);
      line(lines[i].cx, 0, lines[i].cx, height);
...
//      rect(width/2-(screenWidth*screenCount)/2, 0, img.width+screenWidth/2+22, img.height);
  ...
  if (normal) {
    ...
    image(backgroundTwo, width/2, height/2+20);
    ...
    if (doText) {
      ...
      if (sceneIndex==2) {
        textFont(fontMid);
        text(pieces[sceneIndex], width/2-screenWidth*screenCount/2+screenWidth, lines[0].pos.y+screenHeight/8, screenWidth*screenCount-screenWidth*2, screenHeight);
      }
      ...
      if (sceneIndex==39) {
        ...
        lines[0].tpos.y =0;
        lines[1].tpos.y = screenHeight;
        lines[2].tpos.y = screenHeight;
        lines[3].tpos.y = screenHeight;
        lines[4].tpos.y = screenHeight;
        lines[5].tpos.y = screenHeight;
...
void smallText() {
  ...
  if (sceneIndex==11) {
    ...
    lines[7].tpos.y = screenHeight;
    lines[6].tpos.y = screenHeight;
    lines[5].tpos.y = screenHeight;
    lines[4].tpos.y = screenHeight;
    lines[3].tpos.y = screenHeight;
  }
